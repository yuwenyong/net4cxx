...
void GzipFile::initWithOutputFile(std::string fileName, std::ios_base::openmode mode, int compresslevel,
    ...
    if (!(mode & std::fstream::out)) {
        NET4CXX_THROW_EXCEPTION(IOError, "Mode not supported");
    }
    auto fileObj = std::make_shared<std::fstream>();
    fileObj->open(fileName.c_str(), mode | std::fstream::binary);
    _mode = GzipFileMode::WRITE;
    initWrite(fileName);
    _compress = std::make_unique<CompressObj>(compresslevel, Zlib::deflated, -Zlib::maxWBits, Zlib::defMemLevel, 0);
    _fileObj = std::move(fileObj);
    _offset = 0;
    if (mtime) {
        _mtime = *mtime;
...
size_t GzipFile::write(const Byte *data, size_t len) {
    ...
    if (_mode != GzipFileMode::WRITE) {
        NET4CXX_THROW_EXCEPTION(IOError, "write() on read-only GzipFile object");
    }
    if (!_fileObj) {
        NET4CXX_THROW_EXCEPTION(ValueError, "write() on closed GzipFile object");
    }
    if (len > 0) {
        _size += len;
...
ByteArray GzipFile::read(ssize_t size) {
    ...
    if (_mode != GzipFileMode::READ) {
        NET4CXX_THROW_EXCEPTION(IOError, "read() on write-only GzipFile object");
    }
    ByteArray chunk;
...
void GzipFile::rewind() {
    if (_mode != GzipFileMode::READ) {
        NET4CXX_THROW_EXCEPTION(IOError, "Can't rewind in write mode");
    }
...
ssize_t GzipFile::seek(ssize_t offset, std::ios_base::seekdir whence) {
    if (whence == std::ios_base::cur) {
        ...
        NET4CXX_THROW_EXCEPTION(ValueError, "Seek from end not supported");
    }
    if (_mode == GzipFileMode::WRITE) {
        if (offset < _offset) {
            NET4CXX_THROW_EXCEPTION(IOError, "Negative seek in write mode");
        }
...
void GzipFile::readGzipHeader() {
    ...
    if (!_fileObj->good() || magicHeader[0] != '\037' || magicHeader[1] != '\213') {
        NET4CXX_THROW_EXCEPTION(IOError, "Not a gzipped file");
    }
    char method;
    _fileObj->get(method);
    if (!_fileObj->good() || method != '\010') {
        NET4CXX_THROW_EXCEPTION(IOError, "Unknown compression method");
    }
    char flag;
    _fileObj->get(flag);
    if (!_fileObj->good()) {
...
void GzipFile::internalRead(size_t len) {
    if (!_fileObj || _fileObj->eof()) {
        NET4CXX_THROW_EXCEPTION(EOFError, "Reached EOF");
    }
    if (_newMember) {
        ...
        initRead();
        readGzipHeader();
        _decompress = std::make_unique<DecompressObj>(-Zlib::maxWBits);
        _newMember = false;
    }
    ByteArray buf;
    buf.resize(len);
