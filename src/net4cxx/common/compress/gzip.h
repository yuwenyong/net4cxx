...
#ifndef NET4CXX_COMMON_COMPRESS_GZIP_H
#define NET4CXX_COMMON_COMPRESS_GZIP_H
...
class NET4CXX_COMMON_API GzipFile {
public:
    GzipFile() = default;
    ...
    void initWithOutputFile(std::string fileName, std::ios_base::openmode mode=std::fstream::out|std::fstream::trunc,
                            int compresslevel=9, const time_t *mtime= nullptr);
...
protected:
    void checkInited() const {
        if (_mode == GzipFileMode::READ) {
            NET4CXX_THROW_EXCEPTION(IOError, "ReOpen on read-only GzipFile object");
        } else if (_mode == GzipFileMode::WRITE) {
            NET4CXX_THROW_EXCEPTION(IOError, "ReOpen on write-only GzipFile object");
        }
    }

    void checkClosed() const {
        if (!_fileObj) {
            NET4CXX_THROW_EXCEPTION(ValueError, "I/O operation on closed file");
        }
    }
...
#endif //NET4CXX_COMMON_COMPRESS_GZIP_H
